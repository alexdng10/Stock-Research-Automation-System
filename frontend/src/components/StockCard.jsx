...
const StockChart = ({ stock }) => {
  ...
  const validData = React.useMemo(() => {
    ...
    const filteredData = stock.historical_data.dates.reduce((acc, date, index) => {
      // Ensure date is properly formatted as ISO string if it isn't already
      const dateStr = typeof date === 'string' ? date : date.toISOString();
      const parsedDate = new Date(dateStr);
      if (!isNaN(parsedDate) && parsedDate <= today) {
        acc.dates.push(parsedDate.toISOString()); // Store as ISO string
  ...
  const chartData = {
    ...
    datasets: [{
      label: 'Price',
      data: validData.prices.map((price, index) => ({
        x: validData.dates[index],
        y: price
      })),
